...
function requestUrl(req) {
  if (req.url && req.url.indexOf('?') >= 0) return req.url.replace(/^\/api\/rawg/, '/api/games');
  if (req.url && req.query && Object.keys(req.query).length) {
    return (req.url.split('?')[0].replace(/^\/api\/rawg/, '/api/games')) + '?' + new URLSearchParams(req.query).toString();
  }
  return (req.url || '/api/games').replace(/^\/api\/rawg/, '/api/games');
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    res.status(502).json({ error: 'Game catalog proxy failed' });
  }
}

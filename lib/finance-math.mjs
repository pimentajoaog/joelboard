...
/** JSON Detail payload stored on the mine/anchor Debts row. */
export function splitDetailFromState(state, svcOn, svcPct) {
  return {
...
/** Rebuild calculator state from Detail, or people-only fallback rows. */
export function splitStateFromDetail(detail, fallbackPeople) {
  ...
  if (people.length && !people.some(function (p) { return p.isMe; })) people[0].isMe = true;
  return { people: people, items: [], svcOn: false, svcPct: 10 };
}

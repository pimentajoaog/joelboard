...
export async function proxyRecipesRequest(url, env) {
  ...
  /* by=name → título; by=ingredient → filter.php (com singular). Default: name. */
  ...
  try {
    if (by === 'ingredient') {
      ...
      var packs = await Promise.all(variants.map(function (v) {
        return fetchMeals(auth.key, 'filter.php?i=' + encodeURIComponent(v));
      }));
      for (var i = 0; i < packs.length; i++) filtered = filtered.concat(packs[i] || []);
      return {
        status: 200,
        body: JSON.stringify({ results: mergeSearchResults([], filtered), by: by })
      };
    }
    var named = await fetchMeals(auth.key, 'search.php?s=' + encodeURIComponent(q));
    return {
      status: 200,
      body: JSON.stringify({ results: mergeSearchResults(named, []), by: by })
    };
  } catch (e) {
    return {
      status: e.status || 502,

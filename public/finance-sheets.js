...
var JB_COLS = {
  ...
  goals: function(d,id){ return [d.name, parseFloat(d.target), parseFloat(d.current)||0, d.deadline||'', d.color||'#0d9488', id]; },
  ...
  createUserSheet: function(){
    ...
    return jbReq('POST','https://sheets.googleapis.com/v4/spreadsheets', body).then(function(ss){
      ...
      var place = (JB.placeSpreadsheetInAppFolder
        ? JB.placeSpreadsheetInAppFolder(ss.spreadsheetId, 'finance')
        : Promise.resolve());
...
function jbBuildData(t){
  ...
  var goals = jbBody(t.Goals).filter(function(r){ return r[0]; }).map(function(r){ return { id:r[5], name:r[0], target:jbNum(r[1]), current:jbNum(r[2]), deadline:(typeof r[3]==='number'?jbDate(r[3]):(r[3]?String(r[3]):'')), color:r[4] || '#0d9488' }; });

...
const I18N = {
  en: {
    ...
    'fb.attach':'Attachments (optional)', 'fb.attachBtn':'＋ Choose images or videos', 'fb.attachHint':'Up to 10 files · 10 MB each · JPG, PNG, GIF, WebP, MP4, WebM, MOV. Same limits as Google Forms. Sign in with Google to upload attachments.',
  ...
  ptBR: {
    ...
    'fb.attach':'Anexos (opcional)', 'fb.attachBtn':'＋ Escolher imagens ou vídeos', 'fb.attachHint':'Até 10 arquivos · 10 MB cada · JPG, PNG, GIF, WebP, MP4, WebM, MOV. Mesmos limites do Google Forms. Entre com Google para enviar anexos.',
...
function boot(data) {
  ...
  if (!window._finRoute) { window._finRoute = 1; if (JB.onRoute) JB.onRoute(function(){ try{ financeApplyRoute(); renderAll(); }catch(_){ } }); }
...
function financeSyncRoute(replace){
  ...
  var ym=financeYm();
  var nowYm=ymStr(now.getFullYear(), now.getMonth());
  JB.qsPatch({
    tab: currentTab==='overview'?null:currentTab,
    ym: ym===nowYm?null:ym
  }, { replace: !!replace });
}
function financeApplyRoute(){
  var t=JB.qsGet?JB.qsGet('tab'):'';
  var ym=JB.qsGet?JB.qsGet('ym'):'';
  if(ym && /^\d{4}-\d{2}$/.test(ym)){
    var a=ym.split('-'); selY=Number(a[0]); selM=Number(a[1])-1; editingHoursDate=null; setDefaultDate();
  }
  if(!(t && ['overview','worklog','money','bills','budget'].indexOf(t)>-1)) t='overview';
  currentTab = t;
  document.querySelectorAll('.tab-btn').forEach(b=>b.classList.toggle('active', b.dataset.tab===t));
  document.querySelectorAll('.tab-page').forEach(p=>p.classList.toggle('active', p.id==='tab-'+t));
}
function applyDim() {
...
function mountGoalColor(hex) {
  ...
  const v = /^#[0-9a-fA-F]{6}$/.test(hex) ? hex : '#0d9488';
...
function goalColorValue() {
  ...
  return el.getAttribute('data-hex') || '#0d9488';
}
function openGoal() {
  closeFab(); editing={type:null,id:null};
...
function editGoal(id) {
  ...
  mountGoalColor(/^#[0-9a-fA-F]{6}$/.test(g.color)?g.color:'#0d9488');
  document.getElementById('goalOverlay').classList.add('open');
}
function submitGoal() {
...
var FIN_TOUR=[
  ...
  { go:function(){ switchTab('bills'); }, sel:'#tab-bills', title:'Contas & Poupança', body:'Contas recorrentes, metas, rachar conta e pacotes de pagamento — marque conforme paga.' },
...
let fbKind='bug';
var fbFilePickReady=false;
function fbLinksText(links){
  if (!links || !links.length) return '';
  return '\n\nAnexos:\n' + links.map(function(l){ return '• ' + l.name + ': ' + l.url; }).join('\n');
}
function ensureFbFilePick(){
  if (fbFilePickReady || !window.JB || !JB.initFilePick) return;
  var el=document.getElementById('fbFilePick');
  if (!el) return;
  JB.initFilePick(el, { onError: function(err){ setFormError('fbErr', err ? ((err && err.message) || t('fb.attachErr')) : ''); } });
  fbFilePickReady=true;
}
function openFeedback(kind) {
  ...
  ensureFbFilePick();
  if (window.JB && JB.resetFilePick) JB.resetFilePick(document.getElementById('fbFilePick'));
...
function submitFeedback() {
  ...
  const filePick=document.getElementById('fbFilePick');
  const picked=(window.JB && JB.getFilePickFiles) ? JB.getFilePickFiles(filePick) : [];
  const v=(window.JB && JB.fbValidateFiles) ? JB.fbValidateFiles(picked) : { ok:true, files:[] };

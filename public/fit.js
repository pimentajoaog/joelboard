...
function offerPick(files, err){
  var hint = (err&&err.fromFolder)
    ? ('Escolha a planilha em Joelboard/' + (err.folderName||'Fit') + '.')
    : 'Encontramos mais de uma planilha possível.';
  loadingHtml(JB.sheetPickHtml(files, {
    title: 'Qual planilha de treino?',
    hint: hint,
    pickCall: 'pick',
    otherCall: 'gate()',
...
function progPreviewHtml(exId){
  ...
  return '<div class="ppv"><div class="ppvh"><span class="ppvt">Próxima progressão</span><span class="modebadge">'+modeLabel(mode)+'</span></div>'
    ...
    +bar+ladder
    +'<div class="ppvnextlbl">Próximo passo</div><div class="ppvnext">'+progNextMsg(sg,t)+'</div>'
    +'<p class="legal-note">Sugestão automática — não substitui educador físico. <a href="/aviso.html#fit-progressao" target="_blank" rel="noopener">Saiba mais</a></p></div>';
}
function renderProgresso(){
  ...
  var volSec='<div class="secbar"><div class="sect">Volume da semana</div><span class="rg" style="font-size:11px">séries / grupo</span></div><div class="volwrap">'+volPanelHtml()+'</div><p class="legal-note">Metas editáveis — referência geral, não prescrição de treino. <a href="/aviso.html#fit" target="_blank" rel="noopener">Aviso legal</a></p>';
...
function renderVolGoals(){ if(!$('setVol')) return; var tags=(DATA.config&&DATA.config.tags)||DEFAULT_TAGS; $('setVol').innerHTML='<div class="rg" style="font-size:11px;margin-bottom:8px">Padrão 10–20 séries/semana (referência editável — <a href="/aviso.html#fit" target="_blank" rel="noopener">aviso legal</a>).</div>'+tags.map(function(g,i){ var mm=volGoal(g); return '<div class="volgrow"><span class="volgname">'+esc(g)+'</span><input class="field vmini" type="number" inputmode="numeric" value="'+mm[0]+'" onchange="setVolGoal('+i+',0,this.value)"><span class="vdash">–</span><input class="field vmini" type="number" inputmode="numeric" value="'+mm[1]+'" onchange="setVolGoal('+i+',1,this.value)"></div>'; }).join(''); }

...
(function () {
  ...
  function parseNoteImgClipboard(html) {
    var s = String(html || '');
    if (s.indexOf('data-jb-file') < 0) return [];
    var out = [];
    var re = /<img\b[^>]*>/gi;
    var m;
    while ((m = re.exec(s))) {
      var tag = m[0];
      if (/data-jb-ink\s*=\s*["']?1/i.test(tag)) continue;
      var idm = tag.match(/data-jb-file\s*=\s*["']([^"']+)["']/i);
      var id = safeDriveFileId(idm && idm[1]);
      if (!id) continue;
  ...
  var SHEET_VIEWPORT = 'width=device-width, initial-scale=1, minimum-scale=0.25, maximum-scale=5, user-scalable=yes, viewport-fit=cover';
  var BOARD_EDGE_BUMP = 96;
  var BOARD_MAX_PAD = 8000;
  var INK_TOUCH_SLOP = 8;
  var IMG_TOUCH_SLOP = 14;
  ...
  function inkStickyCanvas() {
    if (typeof window === 'undefined' || !window.matchMedia) return false;
    try {
      return window.matchMedia('(pointer: coarse)').matches || window.matchMedia('(max-width: 540px)').matches;
    } catch (_) {
      return false;
    }
  }
  ...
  function mount(host, opts) {
    ...
    function insertClonedNoteImg(meta) {
      ...
      if (live && (live.currentSrc || live.src)) {
        ...
        if (live.getAttribute('data-jb-hydrated')) img.setAttribute('data-jb-hydrated', meta.id);
      }
      if (selectedImg && selectedImg.parentNode) {
        var after = document.createRange();
        after.setStartAfter(selectedImg);
        after.collapse(true);
        try { after.insertNode(img); } catch (_) { insertNode(img); }
      } else {
        restoreSelection();
        insertNode(img);
      }
    ...
    function beginImgPointerDrag(ev, img) {
      if (inkOpen || ev.button || !img) return false;
      ...
      imgDrag = { img: selectedImg, x: ev.clientX, y: ev.clientY, moved: false, dropKey: '', touch: touch };
      imgResize = null;
      return true;
    }
    function clipboardField(ev) {
      var t = ev && ev.target;
      return !!(t && (t.tagName === 'INPUT' || t.tagName === 'TEXTAREA'));
    }
    function clipboardInEditor(ev) {
      if (destroyed || inkOpen) return false;
      if (clipboardField(ev)) return false;
      var t = ev && ev.target;
      if (root.contains(t) || t === imgFrame) return true;
      if (document.activeElement === surface || root.contains(document.activeElement)) return true;
      return false;
    }
    function noteImg(el) {
    ...
    function rangeKey(range) {
      ...
      var id = n.nodeType === 3 ? 't:' + String(n.nodeValue || '').slice(0, 24) : (n.tagName || 'n');
      return id + ':' + range.startOffset;
    }
    function isDropBlock(el) {
      return !!(el && el !== surface && /^(P|DIV|H1|H2|H3|LI|BLOCKQUOTE|PRE)$/.test(el.tagName));
    }
    function skipDropEl(el, img) {
      if (!el) return true;
      if (el === img || el === imgSlot || el === imgGhost || el === imgFrame || el === inkCanvas) return true;
      if (img && img.contains(el)) return true;
      if (el.classList && (el.classList.contains('jb-ed-img-slot') || el.classList.contains('jb-ed-img-ghost'))) return true;
      return false;
    }
    function nestedDropBlocks(el, img) {
      var list = [];
      if (!el || !el.children) return list;
      var kids = el.children;
      var i;
      for (i = 0; i < kids.length; i++) {
        if (isDropBlock(kids[i]) && !skipDropEl(kids[i], img) && kids[i].tagName !== 'IMG') list.push(kids[i]);
      }
      return list;
    }
    function childBlockAtY(parent, y, img) {
      var kids = nestedDropBlocks(parent, img);
      var best = null;
      var bestDist = Infinity;
      var i;
      for (i = 0; i < kids.length; i++) {
        var br = kids[i].getBoundingClientRect();
        if (y >= br.top && y <= br.bottom) return kids[i];
        var dist = y < br.top ? br.top - y : y - br.bottom;
        if (dist < bestDist) {
          bestDist = dist;
          best = kids[i];
        }
    ...
    function hitSurfaceEl(x, y, img) {
      var stack = [];
      try { stack = document.elementsFromPoint(x, y) || []; } catch (_) { stack = []; }
      var i;
      for (i = 0; i < stack.length; i++) {
        var el = stack[i];
        if (skipDropEl(el, img)) continue;
        if (el === surface || surface.contains(el)) return el;
      }
      return null;
    }
    function rangeBeforeAfter(el, after) {
      var r = document.createRange();
      if (after) r.setStartAfter(el);
      else r.setStartBefore(el);
      r.collapse(true);
      return r;
    }
    function computeDropRange(x, y, img) {
      var range = caretRangeAt(x, y);
      var node = range && surface.contains(range.commonAncestorContainer) ? range.startContainer : null;
      if (node && (node === img || (img && img.contains(node)) || (imgSlot && (node === imgSlot || imgSlot.contains(node))))) node = null;
      var el = node ? (node.nodeType === 1 ? node : node.parentElement) : null;
      if (!el || !surface.contains(el)) el = hitSurfaceEl(x, y, img);
      if (!el || skipDropEl(el, img)) return null;
      var block = el;
      while (block && block !== surface && !isDropBlock(block)) block = block.parentElement;
      if (!block || block === surface) {
        block = childBlockAtY(surface, y, img);
        if (!block) {
      ...
      while (nestedDropBlocks(block, img).length) {
        var inner = childBlockAtY(block, y, img);
        if (!inner || inner === block) break;
        block = inner;
      }
      if (block === img || skipDropEl(block, img)) return null;
      var br = block.getBoundingClientRect();
      var edge = 18;
      var tall = br.height > 72;
      if (tall && range && y > br.top + edge && y < br.bottom - edge) {
        var inside = range.startContainer;
        var host = inside && inside.nodeType === 1 ? inside : (inside && inside.parentNode);
        if (host && block.contains(host)) {
          var at = range.cloneRange();
          at.collapse(true);
          return at;
        }
      }
      return rangeBeforeAfter(block, y >= br.top + br.height / 2);
    }
    function dropRangeAt(x, y, img) {
      if (!img || inkOpen) return null;
      var slotOn = imgSlot && imgSlot.parentNode;
      if (slotOn) imgSlot.style.display = 'none';
      var r = null;
      try { r = computeDropRange(x, y, img); } catch (_) { r = null; }
      if (slotOn) imgSlot.style.display = '';
      if (!r) return null;
      return { range: r, key: rangeKey(r) };
    }
    function ensureImgGhost() {
      ...
      imgGhost.style.cssText = 'position:fixed;z-index:100000;pointer-events:none;margin:0;right:auto;bottom:auto;';
      document.body.appendChild(imgGhost);
    }
    function startImgDragPreview(img, ev) {
      if (!img) return;
      ensureImgGhost();
      imgGhost.innerHTML = '';
      ...
      if (imgFrame) imgFrame.hidden = true;
      if (!imgSlot) {
        imgSlot = document.createElement('span');
        imgSlot.className = 'jb-ed-img-slot';
        imgSlot.setAttribute('aria-hidden', 'true');
      }
      var ir = img.getBoundingClientRect();
      imgSlot.style.width = Math.round(Math.max(64, ir.width || img.offsetWidth || 160)) + 'px';
      imgSlot.style.height = Math.round(Math.max(36, Math.min(72, ir.height || 56))) + 'px';
      imgSlot.innerHTML = '';
    ...
    function paintImgGhost(ev) {
      if (!imgGhost || !ev) return;
      imgGhost.hidden = false;
      imgGhost.style.position = 'fixed';
      imgGhost.style.left = Math.round(ev.clientX) + 'px';
      imgGhost.style.top = Math.round(ev.clientY) + 'px';
      imgGhost.style.right = 'auto';
      imgGhost.style.bottom = 'auto';
      imgGhost.style.margin = '0';
      imgGhost.style.transform = 'translate(14px, 12px) rotate(3deg)';
    }
    ...
    function onImgFrameDown(ev) {
      ...
      beginImgPointerDrag(ev, selectedImg);
    }
    function onImgFrameMove(ev) {
      ...
      if (!imgDrag) return;
      var slop = imgDrag.touch ? IMG_TOUCH_SLOP : 4;
      var dx = ev.clientX - imgDrag.x;
      var dy = ev.clientY - imgDrag.y;
      if (!imgDrag.moved && (Math.abs(dx) > slop || Math.abs(dy) > slop)) {
        if (imgDrag.touch && Math.abs(dy) > Math.abs(dx) * 1.15) {
          imgDrag = null;
          return;
        }
        imgDrag.moved = true;
        ...
        try { imgFrame.setPointerCapture(ev.pointerId); } catch (_) {}
        startImgDragPreview(imgDrag.img, ev);
      }
      if (!imgDrag.moved) return;
      ev.preventDefault();
      paintImgGhost(ev);
      var drop = dropRangeAt(ev.clientX, ev.clientY, imgDrag.img);
      if ((!drop || !drop.range) && imgSlot && !imgSlot.parentNode && imgDrag.img.parentNode) {
        drop = { range: rangeBeforeAfter(imgDrag.img, true), key: 'origin' };
      }
    ...
    function inkViewportOverlay() {
      return inkFullSheet() && !inkStickyCanvas();
    }
    ...
    function syncInkCanvasBox() {
      if (!inkCanvas || !inkViewportOverlay()) return false;
      var vis = inkViewRect();
    ...
    function tickInkView() {
      inkViewRaf = 0;
      if (destroyed || !inkViewportOverlay()) return;
      syncInkCanvasBox();
      var o = inkOriginRect();
    ...
    function syncInkBoard() {
      if (destroyed || compact || inkStickyCanvas() || typeof document === 'undefined') {
        clearInkBoard();
        return;
      }
      var html = document.documentElement;
    ...
    function setInkMode(on) {
      ...
      document.documentElement.classList.toggle('jb-ink-full', inkOpen && inkViewportOverlay());
    ...
    function eraseInkAt(ev) {
      ...
      try {
        var cr = inkCanvas.getBoundingClientRect();
        var origin = inkViewportOverlay() ? inkOriginRect() : cr;
    ...
    function inkChromeTarget(ev) {
      if (!inkViewportOverlay()) return null;
    ...
    function inkEdgeScroll(clientX, clientY, ev) {
      if (compact || !inkViewportOverlay()) return;
      var vis = inkViewRect();
      var touch = ev && ev.pointerType === 'touch';
      var margin = touch ? 18 : 40;
      var step = touch ? 18 : 28;
      var dx = 0;
      var dy = 0;
    ...
    function inkPointerCount() {
      ...
      var k;
      for (k in inkPtrIds) {
        if (Object.prototype.hasOwnProperty.call(inkPtrIds, k)) n += 1;
      }
      return n;
    }
    function inkForgetPointer(id) {
      if (id != null) delete inkPtrIds[id];
      if (!inkPointerCount()) inkPinch = false;
    }
    function inkReleaseCaptured() {
      var k;
      if (!inkCanvas) return;
      for (k in inkPtrIds) {
        if (!Object.prototype.hasOwnProperty.call(inkPtrIds, k)) continue;
        try { inkCanvas.releasePointerCapture(Number(k)); } catch (_) {}
      }
    }
    function inkBeginPinch() {
      inkPinch = true;
      var keep = !!(inkCurrent && inkCurrent.pts && inkCurrent.pts.length >= 4);
      inkCancelFreshStroke();
      inkCurrent = null;
      inkReleaseCaptured();
      if (keep) scheduleInkSave();
    }
    function inkCancelFreshStroke() {
      inkPending = null;
      if (inkErasing && !inkErased) inkErasing = false;
      if (inkCurrent && inkCurrent.pts && inkCurrent.pts.length < 4) {
        if (inkStrokes.length && inkStrokes[inkStrokes.length - 1] === inkCurrent) inkStrokes.pop();
        inkCurrent = null;
        redrawInk();
        scheduleInkBoard();
      }
    ...
    function startInkStroke(ev, origin, tap) {
      ...
      if (origin) {
        var r = inkOriginRect();
        pts.push({ x: origin.x - r.left, y: origin.y - r.top });
      }
      var now = inkPos(ev);
      if (!pts.length || pts[0].x !== now.x || pts[0].y !== now.y) pts.push(now);
      inkCurrent = { color: inkColor, width: inkWidth, pts: pts };
    ...
    function onInkPointerDown(ev) {
      ...
      if (ev.pointerType === 'touch') {
        ...
        inkPending = { id: ev.pointerId, x: ev.clientX, y: ev.clientY };
        return;
      }
      startInkStroke(ev);
    }
    function onInkPointerMove(ev) {
      ...
      if (inkPending && ev.pointerId === inkPending.id) {
        var pdx = ev.clientX - inkPending.x;
        var pdy = ev.clientY - inkPending.y;
        if (pdx * pdx + pdy * pdy < INK_TOUCH_SLOP * INK_TOUCH_SLOP) return;
        var origin = inkPending;
        inkPending = null;
        startInkStroke(ev, origin);
        return;
      }
      if (inkErasing) {
        ev.preventDefault();
        if (eraseInkAt(ev)) inkErased = true;
        inkEdgeScroll(ev.clientX, ev.clientY, ev);
        return;
      }
    ...
    function onInkPointerUp(ev) {
      if (ev && ev.pointerId != null) inkForgetPointer(ev.pointerId);
      inkNudge = { left: 0, right: 0, top: 0, bottom: 0 };
      if (inkPending && (!ev || ev.pointerId === inkPending.id)) {
        var tap = inkPending;
        inkPending = null;
        if (!inkPinch && (!ev || ev.type !== 'pointercancel')) {
          startInkStroke({ clientX: tap.x, clientY: tap.y, pointerId: tap.id, pointerType: 'touch' }, null, true);
        }
      }
      if (ev && ev.type === 'pointercancel') inkCancelFreshStroke();
      if (inkErasing) {
        inkErasing = false;
        if (inkErased) {
          scheduleInkBoard();
    ...
    function onEditorCopy(ev) {
      if (!clipboardInEditor(ev) || !selectedImg) return;
      var html = noteImgToClipboardHtml(selectedImg);
      if (!html || !ev.clipboardData) return;
      ev.preventDefault();
      if (ev.stopImmediatePropagation) ev.stopImmediatePropagation();
      else ev.stopPropagation();
      try {
        ev.clipboardData.setData('text/html', html);
        ev.clipboardData.setData('text/plain', selectedImg.getAttribute('alt') || '');
      } catch (_) {}
    }
    ...
    if (uploadImage) {
      ...
      paintInkTools();
      inkCanvas.addEventListener('pointerdown', onInkPointerDown, { passive: false });
      inkCanvas.addEventListener('pointermove', onInkPointerMove, { passive: false });
    ...
    imgFrame.addEventListener('pointerdown', onImgFrameDown, { passive: false });
    imgFrame.addEventListener('pointermove', onImgFrameMove, { passive: false });
    imgFrame.addEventListener('pointerup', onImgFrameUp);
    imgFrame.addEventListener('pointercancel', onImgFrameUp);
    surface.addEventListener('pointermove', onImgFrameMove, { passive: false });
    surface.addEventListener('pointerup', onImgFrameUp);
    surface.addEventListener('pointercancel', onImgFrameUp);
    if (inkCanvas) {
      sizeInkCanvas();

...
function ensureTipoHeader(){ if(notasGrid['Itens']==null) return Promise.resolve(); return JB.api('GET', ssUrl('/values/'+encodeURIComponent('Itens!1:1'))).then(function(res){ var h=(res.values&&res.values[0])||[]; if(h[6]==='Tipo') return; return JB.api('PUT', ssUrl('/values/'+encodeURIComponent('Itens!G1')+'?valueInputOption=RAW'), { values:[['Tipo']] }); }).catch(function(){}); }
function ensureVenceHeader(){ if(notasGrid['Notas']==null) return Promise.resolve(); return JB.api('GET', ssUrl('/values/'+encodeURIComponent('Notas!1:1'))).then(function(res){ var h=(res.values&&res.values[0])||[]; if(h[7]==='Vence') return; return JB.api('PUT', ssUrl('/values/'+encodeURIComponent('Notas!H1')+'?valueInputOption=RAW'), { values:[['Vence']] }); }).catch(function(){}); }
function gate(){
...
function groupRow(g){ var st=groupStats(g.id); return '<div class="ihdr'+(g.feito?' collapsed':'')+'" data-id="'+g.id+'" data-g="1">'
  +'<button class="ihandle" onpointerdown="dragBegin(event,\''+g.id+'\')" title="Arrastar">⠿</button>'
  +'<button class="gchev" onclick="toggleGroup(\''+g.id+'\')" title="Expandir/recolher">'+(g.feito?'▸':'▾')+'</button>'
...
function addManyItems(lines){ var n=note(openNoteId); if(!n||!lines.length) return; var kd=kindDef(n.tipo); var ord=nextOrd(); var add=lines.map(function(L){ var it={ id:uuid(), notaId:openNoteId, ordem:ord++, texto:L, marcavel:kd.defCheck, feito:false, tipo:'' }; DATA.itens.push(it); return it; }); appendItems(add); touchNote(n); renderItems(); renderUsuals(); toast('✓ '+add.length+(add.length>1?' itens':' item')); }

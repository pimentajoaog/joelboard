...
function selMarqueeIgnore(el){ if(!el||!el.closest) return true; if(el.closest('.ihandle,button,input,textarea,.ichk,.itype,.idel,.gchev,.gadd,.gsub,.fmtbar,.selbar,.lnk,.iadd,.gaddbtn,.uwrap,.ed-head,.ed-sub,.duerow,.ed-actions,.fillbtn,.door,.fab')) return true; if(el.closest('.ihdr')) return true; return false; }
function selMarqueeZone(){ var cont=$('edItems'); if(!cont||!openNoteId) return null; var cr=cont.getBoundingClientRect(); if(cr.height<1) return null; return { top:cr.top, bottom:cr.bottom, left:0, right:document.documentElement.clientWidth }; }
...
function selApplyMarquee(rc, additive, base){ var next=additive?Object.assign({}, base):{}, cont=$('edItems'); if(cont) cont.querySelectorAll('.irow[data-id]:not(.ihide)').forEach(function(row){ var id=row.getAttribute('data-id'); if(id && selRectsHit(rc, row.getBoundingClientRect())) next[id]=1; }); _sel=next; if(selCount()){ if(_selMode) selPaintRows(); else selEnterMode(); } else if(_selMode){ if(selExitIfEmpty()) return; selPaintRows(); } else selPaintMarqueePreview(next); }
function selMarqueeBegin(ev){ if(!openNoteId||_editId||_drag) return; if(!itemsOf(openNoteId).some(function(x){return !isGroup(x);})) return; var cont=$('edItems'); if(!cont) return; var inList=cont.contains(ev.target), inZone=selMarqueeInZone(ev); if(!inList && !inZone) return; if(selMarqueeIgnore(ev.target)) return; var row=ev.target.closest?ev.target.closest('.irow[data-id]'):null; if(row && _selMode && _sel[row.getAttribute('data-id')]) return; _mq={ sx:ev.clientX, sy:ev.clientY, ox:ev.clientX, oy:ev.clientY, on:false, additive:!!ev.shiftKey, baseSel:Object.assign({},_sel), pid:ev.pointerId }; }
...
function selToggle(id){ if(_marqueeDidDrag||_dragDidMove) return; if(_sel[id]) delete _sel[id]; else _sel[id]=1; if(selExitIfEmpty()) return; renderItems(); }
function selAll(){ var its=itemsOf(openNoteId).filter(function(x){return !isGroup(x);}); var all=its.length>0 && its.every(function(x){return _sel[x.id];}); _sel={}; if(!all) its.forEach(function(x){ _sel[x.id]=1; }); if(selExitIfEmpty()) return; renderItems(); }

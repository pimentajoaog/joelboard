...
const src = path.join(extDir, 'refresh');
const out = path.join(extDir, 'joelboard-refresh.zip');

if (!existsSync(src)) {
  console.log('zip-extensions: no refresh extension, skipping');
  process.exit(0);
}
...
try {
  if (process.platform === 'win32') {
    ...
    execSync('zip -rq joelboard-refresh.zip refresh', { stdio: 'inherit', cwd: extDir });
  }
  console.log('zip-extensions: wrote joelboard-refresh.zip');
} catch (e) {
